...
const ProductCard: React.FC<ProductCardProps> = ({ id, image, nombre, descripcion = '', precio, stock, onDelete, onEdit }) => {
  ...
  return (
    <div className="product-card border rounded-lg shadow-lg p-4 text-center">
      ...
      <p className="text-gray-600 text-sm truncate max-w-xs mx-auto" title={descripcion}>{descripcion.length > 100 ? `${descripcion.substring(0, 100)}...` : descripcion}</p>
      ...
      <div className="flex justify-between mt-4">
        <button className="bg-blue-500 text-white px-3 py-1 rounded" onClick={() => onEdit({ id, nombre, descripcion, precio, stock })}>Modificar</button>
